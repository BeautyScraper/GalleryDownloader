...
class SantaEvent(gC.rssImageExtractor):
    ...
    def parseFnc(self,response):
        ...
        # elif len(response.css("a[href*=imagebam]::attr(href)").extract()) > 0:
        ...
        elif len(response.css("a[href*=turboimage]::attr(href)").extract()) > 0:
            ...
            urls = response.css("a[href*=vipr]::attr(href)").extract()
            metaData = {'folderName': response.css("title::text").extract()[0] }
            for url in urls:
                # import pdb; pdb.set_trace()
                yield gC.scrapy.Request(url=url.rstrip(), callback=self.vipErSingleImageLink,meta=metaData, priority=1)
            else:
                if len(urls) > 0:
                    self.removeLine(response.url + "\n", r"D:\Developed\Automation\GalleryDownloader\galleryLinks.opml")
    ...
    def vipErSingleImageLink(self,response):
        imageSrc = response.css(".pic::attr(src)").extract()
        fileNames = [x.split("/")[-1] for x in imageSrc]
        folderName = response.meta['folderName']
        self.check(response,len(imageSrc))
        
        pathlib.Path(self.properName("imageSet\\%s" % folderName)).mkdir(parents=True, exist_ok=True)
        # import pdb; pdb.set_trace()
        self.downloadGalleryGeneric(response, imageSrc,fileNames, "",False,"imageSet\\%s" % folderName) 
