...
class SantaEvent(gC.rssImageExtractor):
    ...
    def streamtape(self,response):
        ...
        for artrticle in response.css('article.type-post'): 
            # breakpoint()
            all_img_links.append(artrticle.css('img::attr(src)').get())
            all_img_name.append(artrticle.css('a::attr(title)').get() +  ' '.join(artrticle.css('img::attr(src)').get().split('/')[4:-1]))
